...
from qmltn.torchmps.utils import init_tensor, svd_flex
from qmltn.torchmps.contractables import SingleMat, MatRegion, OutputCore, ContractableList, \
    EdgeVec, OutputMat
import logging
import os


def left_canonical_R(mps):
    D = mps.shape[1]
    d = mps.shape[3]
    R = np.eye(D)
    n = len(mps)
    for i in range(n):
        A = np.einsum("ij,jkl->ilk", R, mps[i])
        A = np.reshape(A, [-1, D])
...
def right_canonical_R(mps):
    D = mps.shape[1]
    d = mps.shape[3]
    Rt = np.eye(D)
    n = len(mps)
    for i in range(n-1, -1, -1):
        A = np.einsum("ijk,lj->kli", mps[i], Rt)
        A = np.reshape(A, [-1, D])
        A = A/np.linalg.norm(A)
        _, Rt = np.linalg.qr(A)
...
def left_canonical_state(mps):
    ...
    for i in range(n):
        ...
        A = A/np.linalg.norm(A)
        Q, R = np.linalg.qr(A)
        mpsl.append(np.reshape(Q, [D, d, -1]))
    mpsl.append(R)
    return mpsl


def right_canonical_state(mps):
    D = mps.shape[1]
    d = mps.shape[3]
    Rt = np.eye(D)
    n = len(mps)
    ...
    for i in range(n-1, -1, -1):
        ...
        mpsr = [np.reshape(np.transpose(Qt), [D, d, -1])] + mpsr

    mpsr = [np.transpose(Rt)] + mpsr
    return mpsr
...
def entropies_MPS(model):
    ...
    left = (mps.module_list[0].tensor +
            mps.module_list[0].bias_mat.unsqueeze(-1)).detach().numpy()
    right = (mps.module_list[2].tensor +
             mps.module_list[2].bias_mat.unsqueeze(-1)).detach().numpy()
    outsite = mps.module_list[1].tensor.detach().numpy()

    D = model.bond_dim
    d = model.feature_dim
    nout = model.output_dim

    mpsl = left_canonical_state(left)
    mpsr = right_canonical_state(right)

    # Entropy at the middle
    Rm = np.einsum("kl,olr,rs->oks", mpsl[-1], outsite, mpsr[0])
    Rm = Rm/np.linalg.norm(Rm,axis=(1,2),keepdims=True)
    _, s, _ = np.linalg.svd(Rm,full_matrices=False)
    ent = -np.sum(s**2*np.log(s**2), axis=1)
    ents = [ent]
    print(ent)
    ...
    Rl = Rm
    for Qr in mpsr[1:]:
        A = np.einsum("okl,lir->okir", Rl, Qr)
        A = np.reshape(A, [nout, -1, D])
        _, s, vh = np.linalg.svd(A,full_matrices=False)
        ent = -np.sum(s**2*np.log(s**2), axis=1)
        ents.append(ent)
        Rl = np.einsum("ol,olr->olr",s,vh)

    # Left part
    Rr = Rm
    for Ql in reversed(mpsl[:-1]):
        A = np.einsum("lir,ors->olis",  Ql,Rr)
        A = np.reshape(A, [nout, D, -1])
        u, s, _ = np.linalg.svd(A,full_matrices=False)
        ent = -np.sum(s**2*np.log(s**2), axis=1)
        ents = [ent] + ents
        Rr = np.einsum("olr,or->olr",u,s)
...
def entropy_TI_MPS(model,seq_len=60):
    ...
    return np.mean(ents[len(ents)//2])

def entropies_TI_MPS(model, seq_len=25*25):
    # Prepend an EdgeVec and append an OutputMat
    mps = model.core_tensor.detach().numpy()
    init_vec = model.init_vector.vec.detach().numpy()
    term_mat = model.terminal_mat.mat.detach().numpy()
    ...
    # Left canonical form
    A = np.einsum("l,lri->ir", init_vec, mps)
    A = A/np.linalg.norm(A)
    mpsl = []
    for i in range(1, seq_len+1):
        dims = A.shape
        Ql, R = np.linalg.qr(A)
        A = np.einsum("il,lrj->ijr", R, mps)
        A = np.reshape(A, [-1, D])
        A = A/np.linalg.norm(A)
        Dr = R.shape[0]
        mpsl.append(np.reshape(Ql,[-1,d,Dr]))

    ents = []
    A = np.reshape(A,[Dr,d,D])
    Ar = np.einsum("lir,ro->oli",A, term_mat)
    for Ql in reversed(mpsl):
        Ar = Ar/np.linalg.norm(Ar,axis=(1,2),keepdims=True)
        u,s,_ = np.linalg.svd(Ar,full_matrices=False)
        ent = -np.sum(s**2*np.log(s**2),axis=1)
        ents.append(ent)
        Dl = Ql.shape[0]
        u = np.einsum("ora,oa->ora",u,s)
        Ar = np.einsum("lir,ora->olia",Ql,u)
        Ar = np.reshape(Ar,[nout,Dl,-1])
    return ents
...
def entropy_MPS(model):
    ...
    Rl = left_canonical_R(left)
    Rr = right_canonical_R(right)
    Aout = np.einsum("il,olr,rj->oij", Rl, outsite, Rr)
    Aout = Aout/np.linalg.norm(Aout,axis=(1,2),keepdims=True)

    Sl = np.linalg.svd(Rl,full_matrices=False)[1]
    Sr = np.linalg.svd(Rr,full_matrices=False)[1]
    Sm = []
    ent = []
    for i in range(len(Aout)):
        A = Aout[i]/np.linalg.norm(Aout[i])
        s = np.linalg.svd(A,full_matrices=False)[1]
        ent.append(-np.sum(s**2*np.log(s**2)))
        Sm.append(s)
    return np.mean(ent)
